...
public class TypeProductController {
	...
	@ResponseBody
	@RequestMapping(value = "/gettypeProduct",method = RequestMethod.GET, produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<BaseResponse> getType(HttpServletRequest request,
		...
		try {
			List<TypeProduct> listTypeProduct = typeProductService.findAllType();
			List<Object> data = typeProductService.findListTypeProductByIdType(listTypeProduct, id_Type);
	...
	// get list product detail with id_list_product
			@ResponseBody
			@RequestMapping(value = "/getProductDetailByIdTypeProduct",method = RequestMethod.GET, produces = {MediaType.APPLICATION_JSON_VALUE})
			public ResponseEntity<BaseResponse> getProducDetailByIdTypeProduct(HttpServletRequest request,
					@RequestParam(value ="id_TypeProduct") int id_TypeProduct){
				BaseResponse response = new BaseResponse();
				response.setStatus(ResponseStatusEnum.SUCCESS);
				response.setMessage(ResponseStatusEnum.SUCCESS);
				response.setData(null);
				try {
					...
					List<Integer> listObject = productService.findListIdProductByIdTypeProduct(listProducts, id_TypeProduct);
